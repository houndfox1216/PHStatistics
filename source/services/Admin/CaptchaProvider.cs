...
namespace PHStatistics.Services.Admin {
    ...
    public static class CaptchaProvider {
        ...
        static CaptchaProvider() {
            var installedFonts = SystemFonts.Families.ToArray();
            ...
            Factory = new CaptchaFactory(TimeSpan.FromMinutes(3), fontFamilies, null, null, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".ToArray(), 20, 50);
        }
        ...
        /// <returns>驗證碼</returns>
        public static Captcha GetCaptcha(int length, float maxFontSize, float minFontSize = 12, string token = null) => Factory.GetCaptcha(length, maxFontSize, minFontSize, token);
