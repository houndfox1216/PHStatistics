...
namespace EmptyProject.Services.Admin.Controllers {
    ...
    public class UserController : ApiController<ServiceUser, UserModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public UserController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<User>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, User>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var entity = Model.Find(values.FromJson<User>().Id);
        ...
        [Authorize(typeof(ServiceUser))]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus>))]
        public JsonResponse ChangePassword(string oldPassword, string newPassword) {
            try {
                Model.ChangePassword(oldPassword, newPassword);
                return Json(ResponseStatus.OK, null, "密碼修改成功");
