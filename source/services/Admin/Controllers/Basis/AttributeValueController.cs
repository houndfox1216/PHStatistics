...
namespace PHStatistics.Services.Admin.Controllers {
    ...
    public class AttributeValueController : ApiController<ServiceUser, AttributeValueModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public AttributeValueController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<AttributeValue>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, AttributeValue>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var entity = Model.Find(values.FromJson<AttributeValue>().Id);
