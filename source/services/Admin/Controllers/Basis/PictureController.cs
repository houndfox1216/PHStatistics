...
namespace PHStatistics.Services.Admin.Controllers {
    ...
    public class PictureController : ApiController<ServiceUser, PictureModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public PictureController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<Picture>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, Picture>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var entity = Model.Find(values.FromJson<Picture>().Id);
