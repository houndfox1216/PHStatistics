...
namespace EmptyProject.Services.Admin.Controllers {
    ...
    public class CategoryController : ApiController<ServiceUser, CategoryModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public CategoryController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<Category>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, Category>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var entity = Model.Find(values.FromJson<Category>().Id);
        ...
        public JsonResponse Delete(int id) {
            try {
                var entity = Model.Find(id);
                Model.Delete(entity.Id, SystemUser.Default);
                return Json(ResponseStatus.OK, entity);
            } catch (FrameworkException fe) {
                return Json(ResponseStatus.InternalServerError, fe, fe.Message);
            } catch (Exception e) {
