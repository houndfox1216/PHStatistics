...
namespace EmptyProject.Services.Admin.Controllers {
    ...
    public class PackedContentController : ApiController<ServiceUser, PackedContentModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public PackedContentController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<PackedContent>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, PackedContent>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var entity = Model.Find(values.FromJson<PackedContent>().Id);
