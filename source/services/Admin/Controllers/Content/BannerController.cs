...
namespace EmptyProject.Services.Admin.Controllers {
    ...
    public class BannerController : ApiController<ServiceUser, BannerModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public BannerController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<Banner>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, Banner>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var entity = Model.Find(values.FromJson<Banner>().Id);
