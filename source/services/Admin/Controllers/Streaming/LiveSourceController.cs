...
namespace EmptyProject.Services.Admin.Controllers {
    ...
    public class LiveSourceController : ApiController<ServiceUser, LiveSourceModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public LiveSourceController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<LiveSource>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, LiveSource>))]
        ...
        public JsonResponse Update(string values) {
            try {
                var data = values.FromJson<LiveSource>();
