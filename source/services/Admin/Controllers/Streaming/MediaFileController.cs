...
namespace EmptyProject.Services.Admin.Controllers {
    ...
    public class MediaFileController : ApiController<ServiceUser, MediaFileModel, Culture> {
        /// <summary>
        /// 建構
        /// </summary>
        public MediaFileController() : base("System") { }
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, List<MediaFile>>))]
        ...
        /// <response code="200">請求已被處理，處理結果以 JSON 型態回應</response>
        [HttpGet("{id}")]
        [Produces("application/json", Type = typeof(JsonResponse<ResponseStatus, MediaFile>))]
        ...
        public JsonResponse Create() {
            try {
                var data = GetParameter<MediaFile>();
        ...
        public JsonResponse Update(string values) {
            try {
                var data = values.FromJson<MediaFile>();
