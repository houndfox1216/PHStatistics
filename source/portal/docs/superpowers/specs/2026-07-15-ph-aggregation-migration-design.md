...
以下兩項是 Task 2 執行比對測試（`PhAggregationComparisonTests`，920 筆 PH 人數表）時額外發現的落差，root cause 排除後確認並非設定錯誤，而是新引擎相對舊程式碼的另外兩種真實行為差異，已於 2026-07-15 呈報使用者並經明確確認接受：

3. **總人數（Id 67）**：舊程式碼的「總人數」對附掛在該人數表上的所有非 `IsSum` 項目做零部門篩選加總，因而誤把 4 筆本應屬於另一個 `StudentPopulationType`（部門 27「數學班」、`Course.Type=1`，課程 Id 154/155/156）的跨型別 `Class`/`Course` 髒資料算了進去（4 筆人數表：2272、2279、2281、2297）。新引擎依 `SourceDepartmentIds` 正確限定來源部門，排除掉這批髒資料，屬於附帶修正而非退化，允許不同。
4. **與上週相比（Id 35/62）**：新引擎的 `DiffWithLastWeek` 依賴 `LastWeekNumber` 快取欄位（依 `Class.Id` 在兩週之間比對同步，見 `StudentPopulationController.cs:216-220`），但 PH 個別指導（EM1）課程每次週匯入都會替每位在籍學生重新建立一筆全新的 `Class`（`Class.Id` 不會延續到下一週），導致快取同步比對不到、算出的 `LastWeekNumber` 加總與上週實際存的總數有落差（5 筆人數表：2272、2274、2279、2281、2287）。舊程式碼是即時查詢上週那張人數表的課程 33/60 加總，沒有這個問題。新引擎的落差屬已知且可接受的範圍，允許不同。

其餘 17 個課程理論上應該 0 落差；若出現非上述四類的落差，代表規則設定或引擎理解有誤，需要回頭檢討。
