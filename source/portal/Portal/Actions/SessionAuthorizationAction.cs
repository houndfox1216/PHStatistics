...
namespace PHStatistics.Portal.Actions {
    ...
    /// </summary>
    [Description("認證用戶")]
    internal class SessionAuthorizationAction : ActionBase<DataContext, SystemPermission> {
        /// <summary>
        /// 建構 AuthorizationAction。
        /// </summary>
        public SessionAuthorizationAction(IUser user, DataContext dbContext = null) : base("認證用戶", user, dbContext) { }
        ...
        protected override void OnExecuted(DataContext context) {
            ...
            var member = context.Member.SingleOrDefault(e => e.Account == account) ?? throw new SecurityException("帳號或密碼不正確");
            Result = member;
        }
    }
}