  ...
  <div class="grid grid-cols-12 gap-6">
    ...
    <div class="intro-y col-span-12 box mt-0 sm:mt-6 p-3">
      ...
      >
        ...
        <DxColumn data-field="Published" caption="已發佈" data-type="boolean" editor-type="dxSwitch" :editor-options="{ switchedOnText: '已發佈', switchedOffText: '未發佈', width: 'auto' }" :width="140" />
        <DxColumn data-field="Ordinal" caption="排序值" data-type="number" :width="100" sort-order="asc" />
        <DxColumn data-field="Picture" caption="圖片" edit-cell-template="picture-edit-template" :visible="false" />
        <template #picture-edit-template="{ data: cell }">
          <fieldset class="dx-static-label-border">
            ...
            <MultilingualImageEditor
              :culture="culture"
              :value="cell.data.Picture"
              @change="(value: any) => grid.instance.cellValue(cell.rowIndex, cell.column.dataField, value)"
              @error="(message: any) => $send('error', message)" 
            >
              <template #default="{ image, change }">
                <ImageEditor 
                  current-path="images"
        ...
        <DxColumn data-field="CategoryId" caption="類別" :visible="false">
          <DxLookup :data-source="categoryStore" :search-enabled="true" search-expr="Name" display-expr="Name" value-expr="Id" />
        </DxColumn>
        <DxColumn data-field="TagIds" caption="相關標籤" edit-cell-template="tags-edit-template" :visible="false" />
        <template #tags-edit-template="{ data: cell }">
          <fieldset class="dx-static-label-border">
            ...
            <DxTagBox
              ...
              display-expr="Name"
              value-expr="Id"
              v-model="cell.data.TagIds"
              @value-changed="(e: any) => grid.instance.cellValue(cell.rowIndex, cell.column.dataField, e.value)"
            />
          </fieldset>
        </template>
        <DxColumn data-field="Introduction" caption="簡介" edit-cell-template="introduction-edit-template" :visible="false" />
        <template #introduction-edit-template="{ data: cell }">
          <fieldset class="dx-static-label-border">
            <legend class="dx-static-label">{{ cell.column.caption }}</legend>
            <MultilingualTextEditor 
              :culture="culture" 
              :value="cell.data.Introduction"
              @change="(value: any) => grid.instance.cellValue(cell.rowIndex, cell.column.dataField, value)"
            >
              <template #="{ text, change }">
                <DxTextArea
...
export default defineComponent({
  ...
  async beforeMount() {
    this.newsStore = await this.$model.dispatch('news/getStore');
