<template>
  <div class="grid grid-cols-12 gap-6">
    ...
    <div class="intro-y col-span-12 box mt-0 sm:mt-6 p-3">
      ...
      >
        <DxStateStoring :enabled="true" type="localStorage" storage-key="media-file-grid" />
        ...
        <DxColumn data-field="Width" caption="寬度" data-type="number" :width="80" />
        <DxColumn data-field="Height" caption="高度" data-type="number" :width="80" />
        <DxColumn data-field="Bitrate" caption="比特率" data-type="number" :width="100" />
        <DxColumn 
          data-field="Duration" 
          caption="片長" 
          :allow-editing="false" 
          alignment="right" 
          :customize-text="(cellInfo: any) => cellInfo.value?.length ? cellInfo.value.slice(0, -8) : cellInfo.value" 
          :editor-options="{ mask: '00:t0:t0', maskRules: { 't': /[0-5]/ } }" 
          :width="80" 
        />
        ...
        <template #thumbnail-edit-template="{ data: cell }">
          <fieldset class="dx-static-label-border">
            ...
            <ImageEditor 
              ...
              :upload="true"
              :download="true"
              :value="getFullThumbnailUrl(cell.value)"
        ...
        <DxEditing :allow-adding="false" :allow-updating="true" :allow-deleting="true" :use-icons="true" mode="popup">
          <DxPopup 
            :show-title="true" 
            :minWidth="708" 
            :width="800" 
            :title="$t('app.streaming.media-file.modal.title')" 
            :resizeEnabled="true"
          ...
          <DxForm label-mode="static" :col-count="1">
            <DxItem :col-count="3" item-type="group">
              ...
              <DxItem :col-span="2" :col-count="2" item-type="group">
                ...
                <DxItem data-field="Bitrate" />
                <DxItem data-field="Duration" />
...
export default defineComponent({
  ...
  setup() {
    ...
    return {
      grid: ref<any>({}),
      player: ref<any>({}),
      mediaFileStore: ref<CustomStore>(),
  ...
  async beforeMount() {
    this.mediaFileStore = await this.$model.dispatch('mediaFile/getStore');
  ...
  methods: {
    ...
    getFullThumbnailUrl(url: string) {
      return url && (url.startsWith("/files/") || url.startsWith("/resources/")) ? `${import.meta.env.VITE_SERVICE_URI}${url}` : url;
    },
    pausePlayer() {
