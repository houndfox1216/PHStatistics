...
/// </summary>
[Description("排序類別資料")]
public class CategoryReorderAction : ActionBase<DataContext, SystemPermission> {
    /// <summary>
    /// 必須具備的系統權限
    /// </summary>
    ...
    public CategoryReorderAction(IUser user, DataContext dbContext = null) : base("排序類別資料", user, dbContext) { }
    ...
    protected override void OnExecuting(DataContext context) {
        ...
        try {
            var sourceRow = dataContext.Category.Find(Parameters.GetValue<int>("source"));
            var targetRow = dataContext.Category.Find(Parameters.GetValue<int>("target"));
            var isAfter = Parameters.GetValue<bool>("isAfter");

            sourceRow.ParentId = targetRow.ParentId;
            sourceRow.Ordinal = targetRow.Ordinal;

            var query = dataContext.Category.Where(e => e.ParentId == targetRow.ParentId && e.Id != sourceRow.Id);
