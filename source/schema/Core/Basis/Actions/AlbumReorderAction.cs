...
/// </summary>
[Description("排序相簿資料")]
public class AlbumReorderAction : ActionBase<DataContext, SystemPermission> {
    /// <summary>
    /// 必須具備的系統權限
    /// </summary>
    ...
    public AlbumReorderAction(IUser user, DataContext dbContext = null) : base("排序相簿資料", user, dbContext) { }
    ...
    protected override void OnExecuting(DataContext context) {
        ...
        try {
            var sourceRow = dataContext.Album.Find(Parameters.GetValue<int>("source"));
            var targetRow = dataContext.Album.Find(Parameters.GetValue<int>("target"));
            ...
            var query = dataContext.Album.Where(e => e.Id != sourceRow.Id);
