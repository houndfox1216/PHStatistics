...
/// </summary>
[Description("排序屬性資料")]
public class AttributeReorderAction : ActionBase<DataContext, SystemPermission> {
    /// <summary>
    /// 必須具備的系統權限
    /// </summary>
    ...
    public AttributeReorderAction(IUser user, DataContext dbContext = null) : base("排序屬性資料", user, dbContext) { }
    ...
    protected override void OnExecuting(DataContext context) {
        ...
        try {
            var sourceRow = dataContext.Attribute.Find(Parameters.GetValue<int>("source"));
            var targetRow = dataContext.Attribute.Find(Parameters.GetValue<int>("target"));
            ...
            var query = dataContext.Attribute.Where(e => e.Id != sourceRow.Id);
