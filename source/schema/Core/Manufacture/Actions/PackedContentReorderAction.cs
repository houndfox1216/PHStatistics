...
/// </summary>
[Description("排序包裝內容物")]
public class PackedContentReorderAction : ActionBase<DataContext, SystemPermission> {
    /// <summary>
    /// 必須具備的系統權限
    /// </summary>
    ...
    public PackedContentReorderAction(IUser user, DataContext dbContext = null) : base("排序包裝內容物", user, dbContext) { }
    ...
    protected override void OnExecuting(DataContext context) {
        ...
        try {
            var sourceRow = dataContext.PackedContent.Find(Parameters.GetValue<int>("source"));
            var targetRow = dataContext.PackedContent.Find(Parameters.GetValue<int>("target"));
            ...
            var query = dataContext.PackedContent.Where(e => e.Id != sourceRow.Id);
